...
if run_engine:
    ...
    with data_tab:
        st.markdown("#### 🧠 Live Neural Factor Streams")
        st.markdown("Explore the entire dataset of raw NLP sentiment predictions generated by the FinBERT architecture.")
        
        # User-friendly ticker filter
        selected_tickers = st.multiselect("Filter by Ticker:", options=tickers, default=tickers)
        
        # Process and sort all data
        df_display = df_news_scored[df_news_scored['Ticker'].isin(selected_tickers)].copy()
        df_display = df_display.sort_values(by='Date', ascending=False).reset_index(drop=True)
        df_display['Date'] = pd.to_datetime(df_display['Date']).dt.strftime('%Y-%m-%d')
        
        # Apply red-to-green styling for sentiment
        styled_df = df_display[['Date', 'Ticker', 'Headline', 'Sentiment_Score']].style.background_gradient(
            cmap='RdYlGn', subset=['Sentiment_Score'], vmin=-1.0, vmax=1.0
